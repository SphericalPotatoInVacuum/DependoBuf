...
void TypeScriptCodeGenerator::PrintVariables(
...
bool TypeScriptCodeGenerator::CheckForTriggers(
...
void TypeScriptCodeGenerator::Generate(const ast::AST *tree) {
  tree_ = tree;
  ...
  auto declaration_generator = [&](const auto &type) {
    if constexpr (std::is_same<std::decay_t<decltype(type)>, ast::Enum>()) {
      if (type.type_dependencies.size() != 0) {
        *output_ << "class " << type.identifier.name << "<";
...
void TypeScriptCodeGenerator::operator()(const ast::Message &ast_message) {
  const std::vector<ast::TypedVariable> checker_input;
  (*this)(ast_message, checker_input);
}

void TypeScriptCodeGenerator::operator()(
  ...
  std::vector<ast::TypedVariable> ts_struct_fields;
  ...
  for (const auto &field : ast_message.fields) {
    ...
    if (!positions_of_variable_dependencies.empty()) {
      ...
      ts_struct_fields.emplace_back(new_field);
    } else {
      ts_struct_fields.emplace_back(field);
    }
  ...
  *output << " {\n";
  ...
  for (auto &field : ts_struct_fields) {
...
void TypeScriptCodeGenerator::operator()(const ast::TypedVariable &variable, bool as_dependency) {
